...
import { User, Bot, Loader2, FilePlus, FilePen, Eye, FolderInput, Trash2, Wrench, type LucideIcon } from "lucide-react";
...
function getToolLabel(toolName: string, args: Record<string, unknown>): string {
  if (toolName === "str_replace_editor") {
    ...
    if (cmd === "str_replace") return `Editing ${path}`;
    if (cmd === "insert") return `Editing ${path}`;
    if (cmd === "view") return `Reading ${path}`;
  }
  if (toolName === "file_manager") {
    const cmd = args?.command as string;
    const path = (args?.path as string) ?? "";
    const newPath = (args?.new_path as string) ?? "";
    if (cmd === "rename") return `Renaming ${path} → ${newPath}`;
    if (cmd === "delete") return `Deleting ${path}`;
  }
  return toolName;
}
...
function getToolIcon(toolName: string, args: Record<string, unknown>): LucideIcon {
  if (toolName === "str_replace_editor") {
    const cmd = args?.command as string;
    if (cmd === "create") return FilePlus;
    if (cmd === "str_replace" || cmd === "insert") return FilePen;
    if (cmd === "view") return Eye;
  }
  if (toolName === "file_manager") {
    const cmd = args?.command as string;
    if (cmd === "rename") return FolderInput;
    if (cmd === "delete") return Trash2;
  }
  return Wrench;
}
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const Icon = getToolIcon(tool.toolName, tool.args as Record<string, unknown>);
                            return (
